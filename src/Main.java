...
public class Main extends PApplet 
{
	...
	public void setup()
	{
		...
		int redStep = (UNPLEASANT_COLOR.getRed() - PLEASANT_COLOR.getRed()) / this.m_Sounds.length;
		int greenStep = (UNPLEASANT_COLOR.getGreen() - PLEASANT_COLOR.getGreen()) / this.m_Sounds.length;
		int blueStep = (UNPLEASANT_COLOR.getBlue() - PLEASANT_COLOR.getBlue()) / this.m_Sounds.length;
		for (int i = 0; i < this.m_Sounds.length; i++)
		{

			m_Sounds [i] = new Sound (random(0, WIDTH), random(0, HEIGHT), random(-15, 15), 
					random (-15,15),
//					i * redStep + PLEASANT_COLOR.getRed(), i * greenStep + PLEASANT_COLOR.getGreen(), 202, 
					random(10,90), this, i);
		}
	}
